...
class Minesweeper():
    ...
    def __init__(self, height=8, width=8, mines=8):
        ...
        self.height = height
        self.width = width
        self.mines = set()                                                      # self.mines is a set of all mine coordinates (for quick membership tests).

        # Initialize an empty field with no mines
        self.board = []                                                         # self.board is a 2D list of booleans: True means a mine.
        ...
        # Add mines randomly                                                    # Mines are placed randomly without duplicates.
        ...
        self.mines_found = set()                                                # can track flags (used to check victory condition).

    def print(self):                                                            # Draws a grid and shows X where mines are (for debugging, not gameplay)
        """
    ...
    def is_mine(self, cell):                                                    # True if (i, j) is a mine
        i, j = cell
        return self.board[i][j]

    def nearby_mines(self, cell):                                               # Counts mines in the 8 neighbors around cell
        """
    ...
    def won(self):                                                              # True if all mines were flagged
        """
...
class Sentence():                                                               # Knowledge Unit
    """                                                                         A Sentence is your AI’s building block: a set of cells and a count of how many in that set are mines.
    ...
    def __init__(self, cells, count):
        self.cells = set(cells)                                                 # e.g., {(1,2), (1,3), (2,3)}
        self.count = count                                                      # e.g., 2
        
    # If you reveal a safe cell (say (r,c)) and the board number is k, then among its unknown neighbors exactly k are mines (after discounting any neighbors you already know are mines). That becomes a sentence.
    ...
    def known_mines(self):                                                      # If count == number of cells (>0), then every cell here is a mine
        """
    ...
    def known_safes(self):                                                      # If count == 0, then every cell here is safe
        """
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):                                       # Called after you click a safe cell and the board returns count (the number around it).
        """
        ...
        """
        This method:
        1- Records the move and marks it safe.
        2- Builds a new sentence about the unrevealed neighbors of cell.
        3- Iteratively deduces any guaranteed safes or mines.
        4- Performs subset inference to generate new constraints.
        5- Repeats until no new information can be derived.
        """
        ...
        # Look at the 8 neighbors of `cell`
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Otherwise add them to sentence if they are in the game board, still-unknown neighbors:
                if 0 <= i < self.height and 0 <= j < self.width:
        ...
        while knowledge_changed:
            ...
            for sentence_1 in self.knowledge:
                for sentence_2 in self.knowledge:
                    ...
                    """
                    Have: {A, B, C} = 2 and {A, B} = 1
                    Then: {C} = 1 (so C is a mine).
                    """
